...
local timeRemoved = 189 --how much time of the song is removed to make a fake duration of the song, so it **looks** short (use seconds as a format), this value cannot be bigger than the song's actual length
local stopFakeBar = 339 --the current StepHit in which u want the bar to go back to normal (the curStep?...), this will work if u are trying to make it go back before the fakeTime ends, otherwise it will automatically go back when time goes to 0
local timeBarRightC = '4B0082' --change what color u want on left bar (hex format)
local timeBarLeftC = '4B0082'  --change what color u want on right bar (hex format)
...
function onCreatePost()	
   if timeBarType ~= 'Disabled' and fakeTime then
      ...
      makeLuaSprite('fakeTimeBar', '', getProperty('timeBar.x')+4.5, getProperty('timeBar.y')+4.5)
      makeGraphic('fakeTimeBar', 1, getProperty('timeBar.height')-9, timeBarLeftC)
   ...
   if curStep > 0 then
      if getProperty('fakeTimeBar.width') < (getProperty('timeBar.width')-9) then 
         if fakeTime and luaSpriteExists('fakeTimeBar') then
            makeGraphic('fakeTimeBar', (getProperty('timeBar.width')-9)*newSongPercent, getProperty('timeBar.height')-9, timeBarLeftC)
            if timeBarType == 'Time Left' then 
